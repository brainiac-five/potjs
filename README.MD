...
| [![Simulation Tests](https://github.com/brainiac-five/potjs/actions/workflows/sim-tests-macos.yml/badge.svg?branch=devtest)](https://github.com/brainiac-five/potjs/actions/workflows/sim-tests-macos.yml) | [![In-Memory Tests](https://github.com/brainiac-five/potjs/actions/workflows/inmem-tests-macos.yml/badge.svg?branch=devtest)](https://github.com/brainiac-five/potjs/actions/workflows/inmem-tests-macos.yml) | | [![Swarm Integration](https://github.com/brainiac-five/potjs/actions/workflows/stress-tests-macos.yml/badge.svg?branch=devtest)](https://github.com/brainiac-five/potjs/actions/workflows/stress-tests-macos.yml) |
...
and a tutorial — and the [function reference](doc/POT_JS_function_reference.pdf).
...
in-memory version of POT that makes developing and testing easy.
...
The inclusion of POT, in the middle, is the same as before.

Next, the anonymous function waits until initialization is complete to create
the key-value store `kvs` that `put()` and `get()` will use. The two
...
#### LOAD KVS
	...
	pot.load(reference : int, [bee node url : string, batch id : string]) ⟶   kvs

Create a new Swarm KVS from a reference that was returned from a previous
'save()'. The parameters are optional. Leaving them out creates a key-value
store in-memory. Giving both conects to a Swarm Bee node to load, and save, the
KVS from and to there.
...
ID are taken from the `new()` call.
...
#### PUT RAW
	...
	kvs.putRaw(key : any, value : any) ⟶   promise of null
...
#### GET RAW
	...
	kvs.getRaw(key : any) ⟶   promise of Uint8Array
...
#### GET BOOLEAN FROM RAW
	...
	kvs.getBoolean(key : any) ⟶   promise of boolean
...
#### GET NUMBER FROM RAW
	...
	kvs.getNumber(key : any) ⟶   promise of number
...
#### GET STRING FROM RAW
	...
	kvs.getString(key : any) ⟶   promise of string
...
## Files
       ...
       doc/                  POT JS documentation
        POT JS manual.pdf    quick start, instructions, tutorial, examples
        POT JS manual.odt
        ...
        test/
         test.html            Test start for in-browser tests
         node.js              Test start for node.js-based tests
         suites.js            Test suites
...
but the KVS caches are gone, all data that was not persisted yet with `save()`.
...
the results from a suite of 160+ tests in `suites.js` across the available put
and get functions. Check out the chapter about tests in the
...
A standard node.js-based test runs with
...
Both in-browser and node.js tests use `test.js` and the `suites.js` suite.

For extended simulation tests, these files are dropped in to replace Go POT:
...
### What You Should See: Standard Test / Node.js
...
```sh
	...
	-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
		...
		Thu Oct 30 2025 02:27:10 GMT+0100 (Central European Standard Time)
	...
	pot:  --------------------------------------------------------------------------------
	pot:  ✦ case #157 » Oversize Value                                      suites.js:4576
	pot:  --------------------------------------------------------------------------------
	...
	pot:  » (re)-using in-memory persister
	pot:  » initialize new P.O.T. - slot 121
	pot:  √ no error raised.                                                         12 ms
	pot:
	...
	pot:
	pot:  ================================================================================
	pot:  ••• Test Suite #16 ••• FAILURES •••                                       IN-MEM
	pot:  ================================================================================
...
### What You Should See: Standard Test / Browser
...
**Web Page**
	...
	✦ case #148 » Immediate Cancel IIsuites.js:3741
